...
def ler_afnd_arquivo(nome_arquivo):
    ...
    for linha in linhas[3:]:
        ...
        if estado_atual not in afnd.estados:
            raise ValueError(f"Estado de origem desconhecido na transição: '{estado_atual}'")
        if proximo_estado not in afnd.estados:
            raise ValueError(f"Estado de destino desconhecido na transição: '{proximo_estado}'")
        
        # Normaliza ε: aceita 'h', 'H' ou 'ε' como epsilon
        if simbolo.lower() == 'h' or simbolo == '\u03b5':
            simbolo_norm = '\u03b5'
        else:
            # Valida alfabeto fixo {0,1}
    ...
    # Define explicitamente o alfabeto conforme o enunciado
    afnd.alfabeto = {'0', '1'}
    return afnd

def converter_afnd_para_afd(afnd, totalizar=False):
    afd = AFND()
    afd.alfabeto = set(afnd.alfabeto)  # {'0','1'}
    
    # Estado inicial do AFD = ε-fecho do estado inicial do AFND
    estado_inicial_afd = afnd.fecho_epsilon({afnd.estado_inicial})
    
    # Mapeia conjuntos de estados (frozenset) -> nome de estado (Q0, Q1, ...)
    mapeamento_estados = {}
    estados_nao_processados = deque()
    contador_estados = 0
    
    # Cria Q0 e define como inicial do AFD
    ...
    while estados_nao_processados:
        ...
        for simbolo in afd.alfabeto:
            ...
            if proximo_conjunto:
                ...
                afd.transicoes[nome_atual][simbolo].add(nome_proximo)
            else:
                if totalizar:
                    if sink_name is None:
                        sink_name = f"Q{contador_estados}"
                        afd.estados.add(sink_name)
                        contador_estados += 1
                    afd.transicoes[nome_atual][simbolo].add(sink_name)
    
    # Se totalizar, adiciona auto-loops no estado poço
    if totalizar and sink_name is not None:
        for s in afd.alfabeto:
            afd.transicoes[sink_name][s].add(sink_name)
    ...
    with open(nome_arquivo, 'w', encoding='utf-8') as f:
        ...
        for estado in sorted(afd.estados):
            for simbolo in sorted(afd.alfabeto):
                for prox_estado in sorted(afd.transicoes[estado][simbolo]):
                    f.write(f"{estado} {simbolo} {prox_estado}\n")

if __name__ == "__main__":
    # Parte 1: Converter AFND-ε (ε representado por h/H/ε) para AFD
    import os
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    entrada = os.path.join(base_dir, 'data', 'entrada_trab_01.txt')
    saida = os.path.join(base_dir, 'out', 'afd_saida.txt')

    afnd = ler_afnd_arquivo(entrada)
    afd = converter_afnd_para_afd(afnd, totalizar=TOTALIZAR_AFD)
    salvar_afd_arquivo(afd, saida)

    print(f"Conversão concluída! AFD salvo em '{saida}'")
